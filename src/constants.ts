...
export const filteredSuggestionsDescription = `Pass an autocompleteParameters object under advancedParameters. This will allow you to pass in any parameters normally supported in the autocomplete endpoint. To see this in action:
- type "white" in the example below. Notice how the user is presented with only "Underwear & socks" results. This is because the results are currently filtered to belong to the "Underwear & Socks" group.
- navigate to the "Filtered Suggestions" story (using the navigation menu to the left), then use the Controls to adjust the values of \`"group_id"\` to \`"W1344079"\`. Next, type "white" in the example autocomplete input field. Notice how the user is presented with only shirt suggestions. This is because we are filtering to the "Shirts" group`;
