...
export const filteredSuggestionsDescription = `Pass a \`filters\` object under \`advancedParameters\` to apply filters to the suggestions. Any parameter supported by our autocomplete endpoint can be passed under \`advancedParameters\`.  To see this in action:
- type "short" in the example below. Notice how the user is presented with only short pants as results. This is because the results are currently filtered to belong to the "Shorts" group.
- navigate to the "Filtered Suggestions" story (using the navigation menu to the left), then use the Controls to adjust the values of \`"group_id"\` to \`"1030"\`. Next, type "short" in the example autocomplete input field. Notice how the user is presented with only short sleeved items as results. This is because we are filtering to the "Shirts" group`;
