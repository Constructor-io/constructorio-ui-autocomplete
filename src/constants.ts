...
export const termsWithFiltersDescription = `Pass a autocompleteParameters filter object under advancedParameters. This will allow you to filter results down. To see this in action:
- type "shirt" in the example below. Notice how the user is presented with only "Black" results.
- navigate to the "Terms With Filters" story (using the navigation menu to the left), then use the Controls to adjust the values of \`"color"\` to \`"Blue"\`. Next, type "shirt" in the example autocomplete input field. Notice how the user is presented with only blue product suggestions.`;
