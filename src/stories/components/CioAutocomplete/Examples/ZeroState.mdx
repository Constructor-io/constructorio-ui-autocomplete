...
### Recent Searches with Links

Use `getSearchResultsUrl` to make recent search items clickable links.

```jsx
<CioAutocomplete
  apiKey="YOUR_API_KEY"
  onSubmit={handleSubmit}
  getSearchResultsUrl={(item) => `/search?q=${encodeURIComponent(item.value)}`}
  zeroStateSections={[
    {
      type: 'recentSearches',
      displayName: 'Recent Searches',
...
Display custom data with full control over the content.
...
Combine recent searches with recommendations.
...
| type | autocomplete, recommendations, recentSearches, or custom | Section type |
| displayName | string | Display name for the section header (required for recentSearches and custom) |
